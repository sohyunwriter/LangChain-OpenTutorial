...
'''
# This FAISS VectorStore extends the LangChain FAISS VectorStore implementation.
# It includes additional functionalities below, which are not present in the original LangChain FAISS vector store.
...
# def _generate_ids(self, texts: List[str], ids: Optional[List[str]]) -> List[str]:
#     """IDs가 없으면 자동으로 UUID 생성"""
#     return ids or [str(uuid.uuid4()) for _ in texts]

class FAISS(DocumentManager):
    ...
    def get_ids(self):
        ...
        (**참고)
        - es: get_documents_ids(self, index_name: str, size: int = 1000) -> List[Dict]
        '''
        return [id_ for idx, id_ in self.index_to_docstore_id.items()] if self.index_to_docstore_id else []
    
    # delete
    def _delete_by_ids(self, ids: List[str] = None, **kwargs: Any
        ) -> Optional[bool]:
        ...
        self.index.remove_ids(np.fromiter(index_to_delete, dtype=np.int64))  # FAISS에서 삭제
        self.docstore.delete(ids)  # 문서 저장소에서 삭제
    ...
    def _delete_by_filter(self, filter: Dict[str, Any] = None, **kwargs: Any
        ) -> Optional[bool]:  # metadata filter
        
        if filter is None:
            raise ValueError(f"No filters are given")
        ...
        for _id in _ids:
            ...
            if not isinstance(doc, Document):
                raise ValueError(f"Could not find document for id {_id}, got {doc}")
            if filter_func(doc.metadata):
                _ids_to_delete.append(_id)
        
        # No documents match the filter
        if not _ids_to_delete:
            raise ValueError("No documents match the filter.")
        
        # Delete the vectors corresponding to the matched document IDs
        self._delete_by_ids(_ids_to_delete)
        return True
    ...
    def delete(
        ...
        **kwargs: Any
        ...
        """Delete by ID or Filters. These are the IDs in the vectorstore and Filters with metadata.
        ...
        if ids is not None and filter is not None:
            raise ValueError("Delete by ids? or Delete by filters? Choose only one")
        
        if ids is None:
            warnings.warn("No specific ids given, so delete all ids")
            self._delete_by_ids(self.get_ids())        
        elif ids is not None:
            self._delete_by_ids(ids)
        elif filter is not None:
            self._delete_by_filter(filter)
        return True
    ...
    def upsert(
        ...
        **kwargs: Any,
        ...
        dup_ids = set(ids).intersection(self.index_to_docstore_id.values())
        if dup_ids:
            self._delete_by_ids(dup_ids) # 중복된 ID 삭제        
        embeddings = self._embed_documents(texts)
        ids = self.__add(texts, embeddings, metadatas=metadatas, ids=ids)
        return
    ...
    def batch_generator(
        ...
        ids: Optional[List[str]] = None, 
        **kwargs
    ) -> Generator[Tuple[List[str], List[Dict], List[str], int], None, None]:
        
        for batch_start in range(0, len(texts), batch_size):
            yield (
                texts[batch_start : batch_start + batch_size],
                metadatas[batch_start : batch_start + batch_size] if metadatas else None,
                ids[batch_start : batch_start + batch_size] if ids else None,
                batch_start
            )
    ...
    def process_batch(
        ...
        batch_metadatas: Optional[List[Dict]] = None, 
        ...
        one batch upsert
        '''
        print(f"Processing batch starting at index {batch_start}")  # 로그 출력 (디버깅용)
        self.upsert(batch_texts, batch_metadatas, batch_ids)
    ...
    def upsert_parallel(
        ...
        **kwargs: Any,
        ...
        """
        Perform a parallel upsert operation.

        Parameters:
            texts (Iterable[str]): List of documents for upsert.
            metadatas:
            ids: 
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = [
                executor.submit(self.process_batch, batch_texts, batch_metadatas, batch_ids, batch_start)
                for batch_texts, batch_metadatas, batch_ids, batch_start in self.batch_generator(
                    list(texts), metadatas, batch_size, ids, **kwargs
                )
            ]
